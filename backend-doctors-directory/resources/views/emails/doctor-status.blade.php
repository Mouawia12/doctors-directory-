...
@php
    ...
    $statusLabel = $isApproved ? 'تمت الموافقة • Approved' : 'تم الرفض • Rejected';
    $ctaAr = $isApproved ? 'عرض ملفك الآن' : 'مراجعة الملاحظات';
    $ctaEn = $isApproved ? 'View your profile' : 'Review the notes';
@endphp
    <div style="max-width:640px;margin:0 auto;background-color:#ffffff;border-radius:24px;padding:32px;box-shadow:0 16px 48px rgba(15,23,42,0.12);">
        ...
        <div style="margin-bottom:20px;">
            <h1 style="margin:4px 0;font-size:22px;color:#0f172a;direction:rtl;text-align:right;">{{ $subjectAr }}</h1>
            <p style="margin:0;color:#334155;font-size:14px;direction:ltr;text-align:left;">{{ $subjectEn }}</p>
        </div>

        <div style="display:grid;grid-template-columns:1fr;gap:16px;margin-bottom:20px;">
            <div style="padding:18px;border-radius:16px;background-color:#f8fafc;border:1px solid #e2e8f0;direction:rtl;text-align:right;line-height:1.7;font-size:15px;color:#0f172a;">
                <p style="margin:0 0 8px;font-weight:700;">مرحباً {{ $user->name ?? 'دكتورنا العزيز' }},</p>
                <p style="margin:0;color:#334155;">{{ $messageAr }}</p>
            </div>
            <div style="padding:18px;border-radius:16px;background-color:#ffffff;border:1px dashed #e2e8f0;direction:ltr;text-align:left;line-height:1.7;font-size:15px;color:#0f172a;">
                <p style="margin:0 0 8px;font-weight:700;">Hello {{ $user->name ?? 'Doctor' }},</p>
                <p style="margin:0;color:#334155;">{{ $messageEn }}</p>
            </div>
        </div>
        ...
        <div style="text-align:center;margin:26px 0;">
            <a href="{{ $portalUrl }}"
               style="display:inline-flex;flex-direction:column;gap:4px;align-items:center;justify-content:center;background:{{ $accentGradient }};color:#ffffff;font-weight:700;padding:14px 26px;border-radius:14px;text-decoration:none;box-shadow:0 10px 30px rgba(37,99,235,0.25);">
                ...
                <span style="font-size:12px;opacity:0.9;">{{ $ctaEn }}</span>
            </a>
        </div>

        <p style="margin-bottom:18px;font-size:13px;color:#475569;text-align:center;line-height:1.6;">
            شكراً لكونك جزءاً من {{ $appName ?? config('app.name') }}. نعمل معك لبناء تجربة علاجية موثوقة للمرضى.<br>
            Thank you for partnering with {{ $appName ?? config('app.name') }} to deliver trusted care for patients.
        </p>
        <div style="border-top:1px solid #e2e8f0;padding-top:14px;font-size:12px;color:#94a3b8;text-align:center;">
            © {{ date('Y') }} {{ $appName ?? config('app.name') }}. جميع الحقوق محفوظة. All rights reserved.
        </div>
    </div>
</body>
</html>
